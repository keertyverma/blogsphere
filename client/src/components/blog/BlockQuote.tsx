...
const BlockQuote = ({ quote, caption = "" }: Props) => {
  ...
  return (
    <div className="bg-primary/20 p-3 pl-5 border-l-4 border-primary rounded-md">
      <blockquote
        className="!text-base lg:!text-lg !leading-6 [&_*]:lg:!text-lg"
        dangerouslySetInnerHTML={{ __html: `&ldquo;${safeQuote}&rdquo;` }}
      />
      {safeCaption && (
        <p
          className="w-full !text-sm md:!text-base mt-2 text-muted-foreground text-right [&_*]:!text-sm [&_*]:lg:!text-base"
